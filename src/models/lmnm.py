...
from src.utils.utils import build_sim, build_non_zero_graph, build_knn_neighbourhood, build_knn_normalized_graph,build_graph_from_adj
...
class LMNM(GeneralRecommender):
    def __init__(self, config, dataset, local_time):
        super(LMNM, self).__init__(config, dataset)
        self.sparse = True
        ...
        self.a = nn.Parameter(torch.tensor(0.5)).cuda()
        ...
        self.dataset_path = os.path.abspath(os.path.dirname(os.getcwd())+config['data_path'] + config['dataset'])
        ...
        mm_attractive_file = os.path.join(self.dataset_path, 'mm_attractive_{}.pt'.format(self.knn_a))
        image_interest_file = os.path.join(self.dataset_path, 'image_interest_{}_{}.pt'.format(self.knn_k, self.knn_i))
        text_interest_file = os.path.join(self.dataset_path, 'text_interest_{}_{}.pt'.format(self.knn_k, self.knn_i))
        ...
        if os.path.exists(image_interest_file) and os.path.exists(text_interest_file):
            ...
            self.image_original_adj1 = image_adj.cuda()
            self.text_original_adj1 = text_adj.cuda()
        ...
        self.image_original_adj = self.image_interest_adj
        self.text_original_adj = self.text_interest_adj
    ...
    def forward(self, adj, train=False):
        ...
        image_weights, text_weights = torch.split(
            self.softmax(
                torch.cat([
                    self.separate_coarse(extended_image_embeds),
                    self.separate_coarse(extended_text_embeds)
                ], dim=-1)
            ),
            1,
            dim=-1
    ...
    def calculate_loss(self, interaction):   #return all_embeds, (integration_embeds, extended_id_embeds), (explicit_image_embeds, explicit_text_embeds)
        users = interaction[0]
